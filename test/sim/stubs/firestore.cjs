...
async function getDocs(colRef) {
  ...
  return {
    ...
    empty: docs.length === 0,
    size: docs.length,
    // Real QuerySnapshot has forEach. Without it here, a caller using
    // forEach throws and any surrounding catch hides the failure.
...
/** Read a document by path, ignoring auth — for assertions only. */
function __peek(path) {
